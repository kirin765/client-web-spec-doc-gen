    ...
    <div className="bg-neutral-50 px-6 py-10">
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        <section className="rounded-2xl bg-navy-950 px-8 py-10 text-white shadow-2xl">
          <p className="text-caption-sm font-semibold uppercase tracking-[0.18em] text-neutral-200">
            전문가 조회
          </p>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {user ? (
              <Link
                to="/quotes"
                className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-3 text-body-sm font-semibold text-secondary-900"
              >
                내 견적서
                <ArrowRight className="h-4 w-4" />
              </Link>
            ) : (
              <span className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-4 py-3 text-body-sm font-semibold text-white">
            ...
            {user?.hasExpertProfile ? (
              <Link
                to="/mypage"
                className="inline-flex items-center gap-2 rounded-xl border border-white/25 px-4 py-3 text-body-sm font-semibold text-white"
              >
                전문가 마이페이지
              </Link>
            ) : null}
        ...
        <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-secondary-100">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-heading-sm font-bold text-secondary-900">전문가 목록</h2>
            <LoadingButton
          ...
          {isLoading ? (
            <p className="mt-6 text-secondary-600">불러오는 중...</p>
          ) : errorMessage ? (
            <p className="mt-6 text-rose-600">{errorMessage}</p>
          ) : developers.length === 0 ? (
            <div className="mt-6 rounded-xl border border-dashed border-secondary-300 p-10 text-center text-secondary-600">
              조회 가능한 전문가가 없습니다.
            </div>
          ) : (
            <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {developers.map((developer) => (
                <article key={developer.id} className="rounded-2xl border border-secondary-200 p-5">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h3 className="text-body-lg font-bold text-secondary-900">{developer.displayName}</h3>
                  ...
                  <p className="mt-3 text-body-sm leading-6 text-secondary-700">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {developer.specialties.slice(0, 3).map((specialty) => (
                      <span key={specialty} className="rounded-full bg-secondary-100 px-3 py-1 text-caption-sm font-medium text-secondary-700">
                        {specialty}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="mt-5">
                    <Link
                      ...
                      className="inline-flex items-center gap-2 rounded-lg bg-primary-600 px-4 py-2 text-body-sm font-semibold text-white"
                    >
