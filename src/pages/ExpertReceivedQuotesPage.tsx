      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <section className="overflow-hidden rounded-[2rem] bg-slate-900 text-white shadow-2xl">
          <div className="grid gap-8 px-8 py-10 lg:grid-cols-[1.3fr_0.7fr] lg:px-12">
            <div>
              <p className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-200">
              ...
              <p className="mt-3 max-w-2xl text-slate-300">{developer.headline}</p>
            ...
            <div className="grid gap-4 rounded-[1.5rem] bg-white/5 p-6 ring-1 ring-white/10">
              <div className="rounded-2xl bg-white/10 p-4">
                <p className="text-sm text-slate-300">총 견적</p>
                <p className="mt-1 text-3xl font-bold">{stats.total}</p>
              </div>
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="rounded-2xl bg-white/10 p-4">
                  <p className="text-slate-300">열람</p>
                  <p className="mt-1 text-2xl font-bold">{stats.viewed}</p>
                </div>
                <div className="rounded-2xl bg-white/10 p-4">
                  <p className="text-slate-300">수락</p>
                  <p className="mt-1 text-2xl font-bold">{stats.accepted}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-100">
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              <p className="text-sm font-semibold text-slate-500">유형</p>
              <p className="mt-2 text-lg font-bold text-slate-900">{developer.type === 'agency' ? '에이전시' : '프리랜서'}</p>
            </div>
            <div>
              <p className="text-sm font-semibold text-slate-500">예산 범위</p>
              <p className="mt-2 text-lg font-bold text-slate-900">
                {formatRange(developer.budgetMin, developer.budgetMax)}
              </p>
            </div>
            <div>
              <p className="text-sm font-semibold text-slate-500">응답 속도</p>
              <p className="mt-2 text-lg font-bold text-slate-900">{developer.avgResponseHours}시간 이내</p>
            </div>
          </div>
        </section>
        ...
        {receivedQuotes.length === 0 ? (
          <section className="rounded-[2rem] border border-dashed border-slate-300 bg-white p-12 text-center">
            ...
            <h2 className="mt-4 text-2xl font-bold text-slate-900">아직 도착한 견적이 없습니다</h2>
            <p className="mt-3 text-slate-600">전문가 목록에서 고객이 견적을 보내면 이 화면에 쌓입니다.</p>
          </section>
        ) : (
          <section className="space-y-4">
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-blue-600">견적 목록</p>
                <h2 className="mt-1 text-2xl font-bold text-slate-900">도착한 견적서</h2>
              </div>
            ...
            <div className="grid gap-4">
              {receivedQuotes.map(({ proposal, project }) => (
                <article
                  key={proposal.id}
                  className="rounded-[1.5rem] bg-white p-6 shadow-sm ring-1 ring-slate-100"
                >
                  ...
                  <div className="mt-5 grid gap-4 md:grid-cols-3">
                    <div>
                      <p className="text-sm text-slate-500">견적 범위</p>
                      <p className="mt-1 text-lg font-bold text-slate-900">
                        {formatRange(proposal.priceMin, proposal.priceMax)}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-slate-500">예상 기간</p>
                      <p className="mt-1 text-lg font-bold text-slate-900">{proposal.durationText}</p>
                    </div>
                    <div>
                      <p className="text-sm text-slate-500">작성 시각</p>
                      <p className="mt-1 text-lg font-bold text-slate-900">
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {proposal.portfolioLinks.length > 0 ? (
                      ...
                      <span className="text-sm text-slate-500">포트폴리오 링크 없음</span>
                    )}
                  </div>
