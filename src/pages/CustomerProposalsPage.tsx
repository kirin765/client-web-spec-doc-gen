    ...
    <div className="bg-secondary-50 px-6 py-10">
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <section className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-secondary-100">
          <p className="text-body-sm font-semibold text-primary-600">고객 제안 비교</p>
          <h1 className="mt-2 text-heading-lg font-bold text-secondary-900">{project.projectName}</h1>
          <p className="mt-3 text-secondary-600">{project.summary}</p>
        ...
        {projectProposals.length === 0 ? (
          ...
          <>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-4">
                      ...
                      {isBestPrice ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-3 py-1 text-caption-sm font-semibold text-amber-700">
                    ...
                    <div className="mt-6 space-y-4 text-body-sm text-secondary-700">
                      <div>
                        <p className="text-secondary-500">가격 범위</p>
                        <p className="mt-1 text-body-lg font-bold text-primary-700">
                          {formatRange(proposal.priceMin, proposal.priceMax)}
                        </p>
                      </div>

                      <div className="flex items-center gap-2">
                        <Hourglass className="h-4 w-4 text-secondary-400" />
                        <span>{proposal.durationText}</span>
                      </div>

                      <div>
                        <p className="text-secondary-500">메시지</p>
                        <p className="mt-1 leading-6">{proposal.message}</p>
                      </div>

                      {proposal.portfolioLinks.length > 0 ? (
                        <div>
                          <p className="text-secondary-500">포트폴리오</p>
                          <div className="mt-2 flex flex-wrap gap-2">
                            {proposal.portfolioLinks.map((link) => (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex items-center gap-1 rounded-full bg-secondary-100 px-3 py-1 text-caption-sm font-medium text-secondary-700"
                              >
                    ...
                    <div className="mt-6 flex flex-wrap gap-2 border-t pt-4">
                      ...
                      <button
                        ...
                        className="rounded-lg bg-emerald-600 px-4 py-2 text-body-sm font-semibold text-white"
                      >
                        수락
                      </button>
                      <button
                        onClick={() => updateProposalDecision(proposal.id, 'rejected')}
                        className="rounded-lg border border-rose-200 px-4 py-2 text-body-sm font-semibold text-rose-700"
                      >
                        거절
                      </button>
                    </div>

                    <div className="mt-3 text-caption-sm font-medium text-secondary-500">
            ...
            <section className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-secondary-100">
              <table className="min-w-full divide-y divide-secondary-200 text-body-sm">
                <thead className="bg-secondary-50">
                  <tr>
                    <th className="px-4 py-3 text-left font-semibold text-secondary-700">전문가</th>
                    <th className="px-4 py-3 text-left font-semibold text-secondary-700">가격</th>
                    <th className="px-4 py-3 text-left font-semibold text-secondary-700">기간</th>
                    <th className="px-4 py-3 text-left font-semibold text-secondary-700">상태</th>
                  </tr>
                </thead>
